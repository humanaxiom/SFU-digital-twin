...
def test_real_artifact_catalog_and_profile_components_match_measured_sidecar():
    ...
    for profile in ALLOWED_MODES:
        ...
        assert all(re.fullmatch(r"[0-9a-f]{12,64}", identifier) for identifier in identifiers)
...
def test_profile_filters_and_component_ids_are_stable_across_insertion_order():
    graph = _base_graph()
    _edge(
        graph,
        (4.0, 0.0, 0),
...
def test_component_mismatch_returns_before_search_and_never_bridges_gap():
    ...
    assert response["origin"]["component_id"] != response["destination"]["component_id"]
...
def test_route_returns_edge_ids_per_level_geometry_steps_and_complete_provenance():
    ...
    assert set(step["type"] for step in response["steps"]) <= {
